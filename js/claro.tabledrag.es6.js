 ...
 *
 * Only Drupal.theme() functions were added.
 *
...
/* eslint-disable default-case, new-cap */
(($, Drupal, drupalSettings) => {
  ...
  $.extend(Drupal.tableDrag.prototype, {
    ...
    makeDraggable(item) {
      ...
      // Create the handle.
      const handle = $(Drupal.theme.tableDragHandle()).addClass('js-tabledrag-handle').attr('title', Drupal.t('Drag to re-order'));
    ...
    row(tableRow, method, indentEnabled, maxDepth, addClasses) {
      ...
      this.changed = false;
      [this.table] = $tableRow.closest('table');
