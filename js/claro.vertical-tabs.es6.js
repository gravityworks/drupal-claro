...
(($, Drupal) => {
  ...
  Drupal.verticalTab.prototype = {
    ...
    focus(triggerFocus = true) {
      ...
      this.title.after($(Drupal.theme.verticalTabActiveTabIndicator())
        .addClass('js-vertical-tab-active'));
    },
    ...
    tabShow() {
      // Display the tab.
      this.item
        .removeClass('vertical-tabs__menu-item--hidden')
        .show();
    ...
    tabHide() {
      // Hide this tab.
      this.item
        .addClass('vertical-tabs__menu-item--hidden')
        .hide();
  ...
  Drupal.theme.verticalTab = (settings) => {
    ...
    tab.item = $('<li class="vertical-tabs__menu-item" tabindex="-1"></li>')
      .append((tab.link = $('<a href="#" class="vertical-tabs__menu-link"></a>')
        .append($('<span class="vertical-tabs__menu-link-content"></span>')
          .append((tab.title = $('<strong class="vertical-tabs__menu-link-title"></strong>').text(settings.title)))
          .append((tab.summary = $('<span class="vertical-tabs__menu-link-summary"></span>'))))));
    return tab;
  };
